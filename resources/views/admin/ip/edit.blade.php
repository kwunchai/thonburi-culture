...
@section('title','แก้ไข IP')

@section('content_header') <h1>แก้ไขข้อมูลทรัพย์สินทางปัญญา</h1> @endsection

@section('content')
<form method="post" action="{{ route('admin.ip.update', $ip) }}" enctype="multipart/form-data" class="card card-body">
@csrf
@method('put')
<div class="row g-3">
  <div class="col-md-6">
    <label class="form-label">ชื่องานสร้างสรรค์ *</label>
    <input name="title" class="form-control" required value="{{ old('title', $ip->title) }}">
  </div>
  <div class="col-md-3">
    <label class="form-label">เลขที่คำขอ</label>
    <input name="application_no" class="form-control" value="{{ old('application_no', $ip->application_no) }}">
  </div>
  <div class="col-md-3">
    ...
    <select name="type" class="form-select" required>
      @foreach([\App\Enums\IpType::INVENTION_PATENT->value, \App\Enums\IpType::PETTY_PATENT->value, \App\Enums\IpType::DESIGN_PATENT->value, \App\Enums\IpType::COPYRIGHT->value, \App\Enums\IpType::TRADEMARK->value, \App\Enums\IpType::GI->value, \App\Enums\IpType::TK->value] as $t)
        <option value="{{ $t }}" @selected(old('type', $ip->type)===$t)>{{ $t }}</option>
      @endforeach
    </select>
  </div>
  ...
  <div class="col-md-6">
    ...
    <input name="applicant_name" class="form-control" value="{{ old('applicant_name', $ip->applicant_name) }}">
  </div>
  <div class="col-md-6">
    <label class="form-label">คณะ</label>
    <input name="faculty" class="form-control" value="{{ old('faculty', $ip->faculty) }}">
  </div>
  <div class="col-12">
    <label class="form-label">ชื่องานวิจัย</label>
    <input name="research_title" class="form-control" value="{{ old('research_title', $ip->research_title) }}">
  </div>

  <div class="col-md-4">
    <label class="form-label">เลขใบรับรอง</label>
    <input name="certificate_no" class="form-control" value="{{ old('certificate_no', $ip->certificate_no) }}">
  </div>
  <div class="col-md-4">
    <label class="form-label">ไฟล์ใบรับรอง (pdf/jpg/png)</label>
    <input type="file" name="certificate" class="form-control">
    @if($ip->certificate_path)
      <small class="text-muted">ปัจจุบัน: <a href="{{ asset('storage/'.$ip->certificate_path) }}" target="_blank">ดูไฟล์</a></small>
    @endif
  </div>
  <div class="col-12">
    <label class="form-label">หมายเหตุ</label>
    <textarea name="remark" class="form-control" rows="3">{{ old('remark', $ip->remark) }}</textarea>
  </div>
</div>
