...
@section('title', 'แก้ไข Slideshow')
@section('header', 'แก้ไข Slideshow')

@section('content')
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title">
            <i class="fas fa-edit"></i> แก้ไขข้อมูล Slideshow
        </h3>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="community_id" class="form-control @error('community_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($communities as $community)
                                        <option value="{{ $community->id }}" {{ old('community_id', $item->community_id) == $community->id ? 'selected' : '' }}>
                                            {{ $community->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="col-md-4">
                    ...
                    <!-- Image Upload -->
                    <div class="form-group">
                        <label>รูปภาพ Slideshow {{ $item->image ? '(เปลี่ยนใหม่)' : '' }}</label>
                        <div class="custom-file mb-3">
                            <input type="file" name="image" 
                                   class="custom-file-input @error('image') is-invalid @enderror" 
                                   id="imageInput"
                                   accept="image/*">
                            <label class="custom-file-label" for="imageInput">{{ $item->image ? 'เปลี่ยนรูปภาพ' : 'เลือกรูปภาพ' }}</label>
                            @error('image')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="publish_date" 
                               ...
                               value="{{ old('publish_date', $item->publish_date->format('Y-m-d')) }}" 
                               required>
                        @error('publish_date')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group" id="featuredOrderGroup">
                                ...
                                <select name="featured_order" class="form-control">
                                    ...
                                    <option value="1" {{ old('featured_order', $item->featured_order) == 1 ? 'selected' : '' }}>1 (แรก)</option>
                                    <option value="2" {{ old('featured_order', $item->featured_order) == 2 ? 'selected' : '' }}>2</option>
                                    <option value="3" {{ old('featured_order', $item->featured_order) == 3 ? 'selected' : '' }}>3</option>
                                    <option value="4" {{ old('featured_order', $item->featured_order) == 4 ? 'selected' : '' }}>4 (สุดท้าย)</option>
                                </select>
                            </div>
